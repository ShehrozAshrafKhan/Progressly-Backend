...
{
    public class GetCompletedTasksQueryHandler : IRequestHandler<GetCompletedTasksQuery, Response<List<TaskDTO>>>
    {
        private readonly IApplicationDbContext _context;
        private readonly ICurrentUserService _currentUser;

        public GetCompletedTasksQueryHandler(IApplicationDbContext context, ICurrentUserService currentUser)
        {
            _context = context;
            _currentUser = currentUser;
        }
        public async Task<Response<List<TaskDTO>>> Handle(GetCompletedTasksQuery query, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (userRole != "USER")
                {
                    ...
                    .Include(t => t.Assignees)
                        ...
                        .Where(x => x.IsCompletedRequest == true && x.IsActive == false && x.Status == TaskStatus.COMPLETED.ToString())
                ...
                else
                {
                    ...
                    var tasks = await _context.Tasks
                            ...
                            .Include(t => t.Assignees)
                                .ThenInclude(a => a.AssignedByUser)
                                .Where(x => x.IsCompletedRequest == true && x.IsActive == false && x.Status == TaskStatus.COMPLETED.ToString() && assignedTaskIds.Contains(x.TaskId))
                            .Select(x => new TaskDTO()
                            {
                                TaskId = x.TaskId,
                                Title = x.Title,
