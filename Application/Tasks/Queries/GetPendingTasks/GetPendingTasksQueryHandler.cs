...
{
    public class GetPendingTasksQueryHandler : IRequestHandler<GetPendingTasksQuery, Response<List<TaskDTO>>>
    {
        private readonly IApplicationDbContext _context;
        private readonly ICurrentUserService _currentUser;

        public GetPendingTasksQueryHandler(IApplicationDbContext context,ICurrentUserService currentUser)
        {
            _context = context;
            _currentUser = currentUser;
        }
        public async Task<Response<List<TaskDTO>>> Handle(GetPendingTasksQuery query, CancellationToken cancellationToken)
        {
            try
            {
               ...
               .Include(t => t.Assignees)
                   ...
                   .Where(x => (x.Status != TaskStatus.COMPLETED.ToString() && x.IsActive == true)
                   || (x.Status == TaskStatus.COMPLETED.ToString() && x.IsActive == true))
               ...
               .Include(t => t.Assignees)
                   .ThenInclude(a => a.AssignedByUser)
                   .Where(x => (x.Status != TaskStatus.COMPLETED.ToString() && x.IsActive == true&& assignedTaskIds.Contains(x.TaskId))
                   || (x.Status == TaskStatus.COMPLETED.ToString() && x.IsActive == true && assignedTaskIds.Contains(x.TaskId)))
               .Select(x => new TaskDTO()
               {
                   TaskId = x.TaskId,
                   Title = x.Title,
